...
class ImageServicePerfTuningSimulation extends Simulation {
  ...
  val imageServiceScenarios = List(
    ...
    GetImage.getImage.inject(
      atOnceUsers(Environment.fiveHundredUsers)
    ...
    PostImage.postImageNoDelete.inject(
      atOnceUsers(Environment.fiveHundredUsers))
  ...
  setUp(imageServiceScenarios)
    .protocols(httpConf)
    .maxDuration(Environment.testDurationFiveMins minutes)
    .assertions(
      ...
      global.failedRequests.percent.lessThan(10)
    )
}